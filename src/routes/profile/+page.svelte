...
<div class="max-w-6xl mx-auto px-4 py-10">
  ...
  <p class="text-gray-600 mb-8">Wybierz jedną z dostępnych opcji.</p>
    ...
    {:else}
      <div class="space-y-3">
        {#each todayClasses as cls}
          <div
            class="w-full border rounded-xl px-4 py-3 flex items-center justify-between gap-4"
          >
            <div class="min-w-0">
              ...
              {#if loadingId === cls.id}
                <div class="text-xs text-blue-600 mt-1">Otwieranie sesji…</div>
              {/if}
            </div>

            <!-- TYLKO TRENER MOŻE OTWIERAĆ SESJĘ -->
            {#if role === "trainer"}
              <button
                class="shrink-0 inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700 hover:bg-blue-200 transition"
