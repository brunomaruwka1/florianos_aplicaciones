...
<div class="max-w-6xl mx-auto px-4 py-10 space-y-8">
  ...
  {#if session && students.length > 0}
    <div
      class="bg-white border rounded-2xl shadow-sm p-4 flex items-center justify-between flex-wrap gap-3"
    >
  ...
  {#if error}
    <div
      class="bg-red-50 border border-red-200 text-red-700 p-4 rounded-2xl text-sm"
    >
      {error}
    </div>
  {/if}
    ...
    <div class="p-6">
      ...
      {:else}
        <div class="space-y-3">
          {#each students as s}
            <div
              class="border rounded-xl p-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"
            >
              ...
              <div
                class="flex flex-col sm:flex-row gap-2 sm:items-center w-full md:w-auto"
              >
                <input
                  placeholder="Ocena"
