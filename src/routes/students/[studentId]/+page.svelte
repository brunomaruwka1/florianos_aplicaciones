<script>
  ...
  const attendanceRows = history.filter(
    (h) => h.present !== null && h.present !== undefined,
  );
  ...
  const gradeRows = history.filter(
  ...
  const gradeAvg =
    gradeCount > 0
      ? Math.round(
          (gradeRows.reduce((sum, h) => sum + Number(h.grade), 0) /
            gradeCount) *
            10,
        ) / 10
      : null;
...
<div class="max-w-6xl mx-auto px-4 py-10 space-y-8">
  <div class="bg-white border rounded-2xl shadow-sm p-6">
    ...
    <div class="text-sm text-gray-600 mt-2 space-y-1">
      <div>
        🎂 Data urodzenia: {new Date(student.birth_date).toLocaleDateString()}
      </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div class="bg-white border rounded-2xl shadow-sm p-5">
      ...
      <div class="text-sm text-gray-600 mt-1">Zapisane w historii ucznia</div>
    </div>
  </div>
  ...
  <div class="bg-white border rounded-2xl shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b bg-gray-50">
      ...
      <p class="text-sm text-gray-500">Obecność + oceny z zajęć.</p>
    </div>
            ...
            >
              <div class="min-w-0">
                <div class="font-semibold truncate">
                  {h.session?.group?.name || "Grupa"} · {formatDatePL(
                    h.session?.session_date,
                  )}
                </div>

                <div class="text-sm text-gray-600 mt-1">
                  {formatTime(h.session?.start_time)} – {formatTime(
                    h.session?.end_time,
                  )}
                </div>
